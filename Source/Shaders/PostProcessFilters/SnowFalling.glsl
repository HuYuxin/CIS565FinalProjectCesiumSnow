...
float flakeVolume()
{
    ...
    for (float r = 1.0; r <= 16.0; r+=0.5)
    {
        ...
        if (delta >= 0.0)
        {
            ...
            s *= smoothstep(snowThicknessThreshold + 0.01, 1.0, texture2D(u_texture, vec2(0.35 * teta * r, 0.1 * p2.y + fall)).r);
            s *= smoothstep(snowThicknessThreshold + 0.05, 1.0, texture2D(u_texture, vec2(-(0.17 * p2.y + fall), 0.4 * teta * r)).r);
            s *= smoothstep(snowThicknessThreshold - 0.05, 1.0, texture2D(u_texture, vec2(0.16 * p2.y + fall, -0.4 * teta * r)).r);
...
vec3 renderEverything(vec2 offset)
{
    ...
    col = (ice.a * 0.4) * ice.rgb + (1.0 - ice.a * 0.4) * col.rgb;
